...
program cans
...
#else
  use mod_utils          , only: bulk_mean,bulk_mean_mod,bulk_mean_mod_sq
#endif
  !@acc use mod_utils    , only: device_memory_footprint
  use mod_types
  use omp_lib
#if defined(_STW)
  !use mod_bcstw          , only: bc_rhs_stw,corr_abc_stw !not needed, for now
#endif
#if defined(_DRL)
  use mod_drl
#endif
  ...
  ! Removing the declaration of time here is needed to declare it in param.f90 and
  ! make it accessible by other routines.
  real(rp) :: dt,dti,dtmax,dtrk,dtrki,divtot,divmax
  !integer :: irk,istep
  integer :: istep
  ...
  if(myid == 0) then
    ...
    !allocate(u_obs_send (1:int(ng(1)/drl_inc*0.75),1:int(ng(2)/drl_inc*0.75)))
    u_obs_us = 0.
  ...
  if(myid == 0) then
    do i=0,mysize-1
      recvcounts(i+1) = bnx*bny
      displs(i+1)     = mod(i,ng(1)/drl_inc/bnx) + i/(ng(1)/drl_inc/bnx)*bny*ng(1)/drl_inc/bnx
    end do
    print*, 'displs = ', displs
  end if
 ...
 pert_amp   = 0.001_rp
  ...
  do while(.not.is_done)
    ...
    if (start_drl) then
      ...
      call drl_read_request(req,time_max,drl_dpdx,offset,u_obs,is_done,kill)
      start_drl = .false.
    end if
#endif
#if defined(_TIMING)
    ...
    if(stop_type(2)) then ! maximum simulation time reached
      if(time  >= time_max*(n_act)) is_done = is_done.or..true.
    end if
...
#if defined (_DRL)
    if(mod(istep,icheck) == 0) then
      call bulk_mean_mod   (n,grid_vol_ratio_f,u         ,drl_ind_mean,meanvelu)
      call bulk_mean_mod   (n,grid_vol_ratio_f,v         ,drl_ind_mean,meanvelv)
      call bulk_mean_mod   (n,grid_vol_ratio_f,w         ,drl_ind_mean,meanvelw)
      call bulk_mean_mod_sq(n,grid_vol_ratio_f,u-meanvelu,drl_ind_mean,e_ks_1  )
      call bulk_mean_mod_sq(n,grid_vol_ratio_f,v-meanvelv,drl_ind_mean,e_kw_1  )
      call bulk_mean_mod_sq(n,grid_vol_ratio_f,w-meanvelw,drl_ind_mean,e_kw_2  )
    ...
    drl_dpdx = drl_dpdx + dpdl(1)*dt/time_max
    if (time.gt.(time_max*n_act)) then
      n_act     = n_act + 1
      is_done   = .false.
      send_drl  = .true.
      start_drl = .true.
    ...
    if (myid == 0) print*, 'send_drl = ', send_drl
    if (send_drl) then
      e_drl = e_ks_rew
      call undersample(u,drl_ind_mean,drl_inc,u_obs,ourid)
      if (myid == 0) print*, 'Obtained observation data'
      !call MPI_GATHER(u_obs,48,MPI_DOUBLE,u_obs_all,48,MPI_DOUBLE,0,canscomm,ierr)
      call MPI_GATHERV(u_obs,    bnx*bny,MPI_DOUBLE,&
                       u_obs_us, recvcounts,displs, custom_type, 0, canscomm, ierr)
      if (ourid==1) then 
        print*, 'Size of u_obs_us = ', size(u_obs_us,1), size(u_obs_us,2)
        call interp_obs(u_obs_us,u_obs_send)
        call MPI_SEND(u_obs_send,size(u_obs_send),MPI_DOUBLE,0,3,intracomm,ierr)
        call MPI_SEND(drl_dpdx,1,                 MPI_DOUBLE,0,2,intracomm,ierr)
        call MPI_SEND(e_ks_rew,1,                 MPI_DOUBLE,0,1,intracomm,ierr)
      end if
