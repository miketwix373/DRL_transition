...
#if defined (_STW)
real(rp), protected :: amp
real(rp), protected :: lambda
#if !defined (_DRL)
real(rp) :: omega
#endif
logical :: do_stw
...
#if defined (_STW)
     open(newunit=iunit,file='stw.nml',status='old',action='read',iostat=ierr)
       if (ierr == 0) then
         read(iunit,nml=stw,iostat=ierr)
       else
         if (myid == 0) print*, 'Error reading STW input file'
         if (myid == 0) print*, 'Aborting...'
         call MPI_FINALIZE(ierr)
         error stop
       end if
     close(iunit)
     !lambda = lambda*2.*pi/l(1)
#endif
#if defined(_OPENACC)
    !
