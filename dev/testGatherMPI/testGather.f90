program gatherv_example
    ...
    integer :: rank, mysize, ourid, oursize, ierr, i
    integer :: custom_type, custom_type_old, typesize
    integer :: parentcomm, intracomm, group, cgroup, ccomm
    integer(kind=mpi_address_kind) :: lb, extent
    integer, allocatable :: recvcounts(:), displs(:)
    integer, allocatable :: local_data(:,:), global_data(:,:)
    integer, allocatable, dimension(:) :: comm_ranks
    integer :: soma
    ...
    allocate(comm_ranks(mysize))
    do i = 1,mysize
      comm_ranks(i) = i-1
    end do
    call MPI_Group_Incl(group,mysize,comm_ranks,cgroup,ierr)
    call MPI_Group_Size(cgroup,mysize,ierr)
    call MPI_Comm_Create(MPI_COMM_WORLD,cgroup,ccomm,ierr)
    ...
    call MPI_Gatherv(local_data, pnx*pny, MPI_INT, &
                     global_data, recvcounts, displs, custom_type, 0, ccomm, ierr)
    ...
    soma = 42
    if (rank==0) call MPI_Send(soma,1,MPI_INT,0,2,intracomm,ierr)
    if (rank==0) call MPI_Send(global_data,nx*ny,MPI_INT,0,3,intracomm,ierr)
